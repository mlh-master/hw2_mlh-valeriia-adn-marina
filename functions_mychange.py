...
def calc_stat(X_test, y_test, logreg, flag = 'log'):
    ...
    if flag == 'log':
        ...
        y_pred_test = np.where(y_pred_test == 'Negative', -1, y_pred_test)
        y_pred_test = np.where(y_pred_test == 'Positive', 1, y_pred_test)
        loss = hinge_loss(y_test, y_pred_test)
